...
class Songs:
    ...
    def get_files(self, *args):
        if not os.path.exists('data/user_data.json') or len(args) == 4:
            ...
            self.read.set_file('data/user_data.json')
    ...
    def get_download_list(self):
        ...
        self.read.set_file('Downloads/songs.txt')
    ...
    def download(self):
        for song in self.download_dict.keys():
            ...
            download_command = 'youtube-dl --extract-audio --audio-format mp3 -o "' + self.output_path + song + '.mp3" '+self.download_dict[song]
            subprocess.call('echo '+self.sudo_pass+' | sudo -S ' + download_command, shell=True)
        self.write.set_file('playlists/universal.txt')
        self.write.write_playlist(self.playlist)
        subprocess.call('echo ' + self.sudo_pass + ' | sudo -S chown -R ' + self.username + ' ' + self.output_path, shell=True)
